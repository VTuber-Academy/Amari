...
export class UserEvent extends Listener {
	public override async run(oldMember: GuildMember, newMember: GuildMember) {
		...
		if (oldMember.flags.missing('CompletedOnboarding') && newMember.flags.has('CompletedOnboarding')) {
	...
	private async screenMember(member: GuildMember) {
		try {
			const staffChannel = (await member.guild.channels.fetch(config.securityGate).catch((err) => {
			...
			const tranquilizerRole = await member.guild.roles.fetch(config.tranquilizerRole).catch((err) => {
